...
class IME_Sqlite3DB extends IME_DB {
  #db;
  #callbacks = new Map();
  #dpIdentificationTable = new Map();
  #dpTypeIdentificationTable = new Map();
  #updateStmt;
  ...
  DpCreate(dpName, dpTypeName) {
    if (Array.isArray(dpName)) {
      if (!Array.isArray(dpTypeName) || dpTypeName.length !== dpName.length) {
        throw new Error('dpTypeName must be an array of the same length as dpName');
      }
      const results = [];
      const transaction = this.#db.transaction(() => {
        for (let i = 0; i < dpName.length; i++) {
          if (!this.isValidDpName(dpName[i])) {
            throw new Error(`Invalid dpName '${dpName[i]}'`);
          }
          if (this.DpExists(dpName[i])) {
            throw new Error(`DataPoint '${dpName[i]}' already exists`);
          }
          if (!this.DpTypeExists(dpTypeName[i])) {
            throw new Error(`DataPointType '${dpTypeName[i]}' does not exist`);
          }
          results.push(this.#createDataPoint(dpName[i], dpTypeName[i], dpName[i], null, true));
        }
      });
      transaction();
      return results;
  ...
  DpSet(name, value) {
    if (Array.isArray(name)) {
      if (!Array.isArray(value) || value.length !== name.length) {
        throw new Error('value must be an array of the same length as name');
      }
      const transaction = this.#db.transaction(() => {
        for (let i = 0; i < name.length; i++) {
          this.#setSingleDp(name[i], value[i]);
        }
      });
      transaction();
    } else {
      this.#setSingleDp(name, value);
    }
  }

  #setSingleDp(name, value) {
  ...
  DpGet(name) {
    if (Array.isArray(name)) {
      const results = [];
      const transaction = this.#db.transaction(() => {
        for (const dpName of name) {
          results.push(this.#getSingleDp(dpName));
        }
      });
      transaction();
      return results;
    } else {
      return this.#getSingleDp(name);
    }
  }
  ...
  #getSingleDp(name) {
    ...
    if (entry.dpType === DpType.STRUCT) {
      ...
      for (const child of children) {
        ...
        result[child.name] = this.#getSingleDp(childPath);
      }
      return result;
    }
  ...
  DpDelete(name) {
    if (Array.isArray(name)) {
      const transaction = this.#db.transaction(() => {
        for (const dpName of name) {
          this.#deleteSingleDp(dpName);
        }
      });
      transaction();
      return true;
    } else {
      return this.#deleteSingleDp(name);
    }
  }
